...
/**
 * Middleware simple de validation des champs requis
 * @param {string[]|object|Function} schemaOrFields - Champs requis ou schema custom
 */
const validateRequest = (schemaOrFields) => (req, res, next) => {
    try {
        ...
        if (
            schemaOrFields &&
            typeof schemaOrFields === 'object' &&
            typeof schemaOrFields.validate === 'function'
        ) {
            ...
            if (error) {
                ...
                logger.warn('Validation schema echouee', {
                    errors,
        ...
        logger.error('Erreur interne de validation', { error: err.message });
        res.status(500).json({
            success: false,
